...
function initQuiz() {
    ...
    // Add event listener for the continue journey button (if it exists)
...
function showResults() {
    ...
    // Wait 3 seconds before showing the results
    setTimeout(() => {
        ...
        // Scroll to top to see results from the beginning
        window.scrollTo(0, 0);
...
// Mini-course invitation text that appears on the form page
...
function generateReflection() {
    ...
    // Save the level to localStorage so it can be accessed on the form page
    localStorage.setItem('userReflectionLevel', level);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Only run form validation if we're on the form page
...
// Start the quiz when the page loads (only if quiz elements exist)
